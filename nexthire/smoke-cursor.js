/**
 * Standalone WebGL Fluid Smoke Simulation (Modular Component)
 * Wrapped in an IIFE to prevent global namespace pollution and animation conflicts.
 */
(() => {
  const config = {
    SIM_RESOLUTION: 128,
  ...
  function initializeWebGL() {
    ...
    if (!canvas) {
      throw new Error("Fluid canvas element (#fluid) not found in DOM.");
    }
  ...
  function init() {
    try {
      ...
      console.error("Failed to initialize WebGL fluid smoke simulation:", error);
      // Gracefully hide fluid-container instead of writing error to HTML body
